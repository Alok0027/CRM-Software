  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4">
        <button type="button" onClick={onCancel} className="w-full sm:w-auto px-6 py-2.5 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 font-medium transition-all duration-200">Cancel</button>
        <button type="submit" className="w-full sm:w-auto px-6 py-2.5 rounded-lg bg-gradient-to-r from-blue-500 to-blue-600 text-white hover:shadow-lg font-medium transition-all duration-200 shadow-md">{lead ? 'Update Lead' : 'Add Lead'}</button>
      </div>
    </form>
  );
};
