    ...
    <section className="py-16 sm:py-24 bg-white font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-4xl mx-auto mb-12 sm:mb-16">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-medium text-gray-800 mb-4">Flexible Pricing Plans</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-start">
          {pricingPlans[billingCycle].map((plan, index) => (
            <div key={index} className={`bg-gray-50 rounded-lg p-6 sm:p-8 shadow-sm transition-all duration-300 ${plan.popular ? 'border-2 border-orange-500 shadow-xl' : 'border-2 border-transparent'}`}>
              {plan.popular && (
              ...
              <div className="mb-6">
                <span className="text-4xl sm:text-5xl font-medium text-gray-800">{plan.price}</span>
        ...
        <div className="mt-24">
          <h3 className="text-3xl sm:text-4xl font-medium text-gray-800 text-center mb-12">Feature Comparison</h3>
          <div className="border border-gray-200 rounded-lg overflow-x-auto">
            <div className="hidden sm:grid grid-cols-4 gap-4 bg-gray-50 p-4 md:p-6 font-medium text-base md:text-lg text-gray-800">
            ...
            <div className="sm:hidden">
              {pricingPlans.monthly.map((plan, planIndex) => (
                <div key={plan.name} className="p-4 border-b last:border-b-0">
                  <h4 className="font-semibold text-lg mb-4 text-center text-white bg-orange-500 rounded-md py-2">{plan.name}</h4>
                  {featureComparison.map((categoryItem, catIndex) => (
                    <div key={catIndex} className="mb-4">
                      <div className="flex items-center mb-2">
                        <categoryItem.icon className="text-orange-500 mr-2" />
                        <h5 className="font-medium text-gray-800">{categoryItem.category}</h5>
                      </div>
                      <ul>
                        {categoryItem.features.map((feature, featIndex) => (
                          <li key={featIndex} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
            ...
            <div className="hidden sm:block">
              {featureComparison.map((categoryItem, catIndex) => (
                <div key={catIndex}>
                  <div className="grid grid-cols-4 gap-4 items-center p-4 md:p-6 bg-gray-100 border-b border-gray-200">
                  ...
                  {categoryItem.features.map((feature, featIndex) => (
                    <div key={featIndex} className="grid grid-cols-4 gap-4 items-center p-4 md:p-6 text-center border-b border-gray-200 last:border-b-0">
                      ...
                      {feature.included.map((isIncluded, planIndex) => (
                        <div key={planIndex} className="flex justify-center">
                          {isIncluded ? (
                            <FaCheck className="text-orange-500 text-xl md:text-2xl" />
                          ) : (
                            <span className="text-gray-400 font-medium text-xl md:text-2xl">-</span>
                          )}
                        </div>
                      ))}
                    </div>
